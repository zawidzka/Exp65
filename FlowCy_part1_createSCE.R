...
FCSfiles <- FCSfiles[!grepl('TPEX_D7', FCSfiles)]
FCSfiles <- FCSfiles[!grepl('TEFF_D7', FCSfiles)]
...
sample_md <- sample_md[!grepl('_D7_', sample_id)]
sample_md <- sample_md[!grepl('BM_TTERM', sample_id)]
sample_md <- sample_md[-23]
sample_md


is.data.table(sample_md)
sample_md[, sample_id := factor(sample_id)]
# sample_md[, condition := factor(condition, levels = c("BM_TEFF_D7", "BM_TPEX_D7", "BM_TTERM_D7",
#                                                     "BM_TEFF_D28", "BM_TPEX_D28", "BM_TTERM_D28",
#                                                      "TUM_TEFF", "TUM_TPEX", "TUM_TTERM"))]
                         
# sample_md[ , condition := factor(condition, levels = c("BM_D7", "BM_D28", "LN_D7", "LN_D28", 
#                                                         "SP_D7","SP_D28", "TUM_D28"))]

sample_md[, condition := factor(condition, levels = c("BM_D7", "BM_D14", "BM_D21", "BM_D28",
                                                      "TUM_D14", "TUM_D21", "TUM_D28"))]
...
chs_of_interest <- colnames(flowSet)[10:27]
...
saveRDS(sce, file = "SCE_OTI_BMTUM.rds")
